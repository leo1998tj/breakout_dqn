...
def train(shared_model, shared_optimizer, rank, args, info):
    ...
    while info['frames'][0] <= 8e7 or args.test:  # openai baselines uses 40M frames...we'll use 80M
        ...
        for step in range(args.rnn_steps):
            ...
            try:
                fi_score = cal_fi_score(state, model, hx)
            except:
                fi_score = 1
        ...
        torch.save(model.state_dict(), "0829_model_paras.pt".format())
...
if __name__ == "__main__":
    ...
    args.save_dir = '{}_new_0829_1/'.format(args.env.lower())  # keep the directory structure simple
